...
input_dir_cat_dog = file.path("/home/tobynb/shared-folder/tbranck/Pets_pooled_analysis/inputs/cat_dog")
input_dir_human_dog = file.path("/home/tobynb/shared-folder/tbranck/Pets_pooled_analysis/inputs/human_dog")
input_dir_human_cat = file.path("/home/tobynb/shared-folder/tbranck/Pets_pooled_analysis/inputs/human_cat")
...
file_paths_rgnavus <- c(
  file.path(input_dir_cat_dog,"Ruminococcus_gnavus_gene_terms.tsv"),
  file.path(input_dir_human_dog,"Ruminococcus_gnavus_gene_terms.tsv"),
  file.path(input_dir_human_cat,"Ruminococcus_gnavus_gene_terms.tsv")
)
...
file_paths_bwexlerae <- c(
  file.path(input_dir_cat_dog,"Blautia_wexlerae_gene_terms.tsv"),
  file.path(input_dir_human_dog,"Blautia_wexlerae_gene_terms.tsv"),
  file.path(input_dir_human_cat,"Blautia_wexlerae_gene_terms.tsv")
)
...
#### P. copri ####
file_paths_pcopri <- c(
  file.path(input_dir_cat_dog,"Prevotella_copri_gene_terms.tsv"),
  file.path(input_dir_human_dog,"Prevotella_copri_gene_terms.tsv"),
  file.path(input_dir_human_cat,"Prevotella_copri_gene_terms.tsv")
)
...
file_paths_ahadrus <- c(
  file.path(input_dir_cat_dog,"Anaerostipes_hadrus_gene_terms.tsv"),
  file.path(input_dir_human_dog,"Anaerostipes_hadrus_gene_terms.tsv"),
  file.path(input_dir_human_cat,"Anaerostipes_hadrus_gene_terms.tsv")
)
...
file_paths_bstercoris <- c(
  file.path(input_dir_cat_dog,"Bacteroides_stercoris_gene_terms.tsv"),
  file.path(input_dir_human_dog,"Bacteroides_stercoris_gene_terms.tsv"),
  file.path(input_dir_human_cat,"Bacteroides_stercoris_gene_terms.tsv")
)
...
file_paths_buniformis <- c(
  file.path(input_dir_cat_dog,"Bacteroides_uniformis_gene_terms.tsv"),
  file.path(input_dir_human_dog,"Bacteroides_uniformis_gene_terms.tsv"),
  file.path(input_dir_human_cat,"Bacteroides_uniformis_gene_terms.tsv")
)
...
file_paths_bvulgatus <- c(
  file.path(input_dir_cat_dog,"Bacteroides_vulgatus_gene_terms.tsv"),
  file.path(input_dir_human_dog,"Bacteroides_vulgatus_gene_terms.tsv"),
  file.path(input_dir_human_cat,"Bacteroides_vulgatus_gene_terms.tsv")
)
...
file_paths_blongum <- c(
  file.path(input_dir_cat_dog,"Bifidobacterium_longum_gene_terms.tsv"),
  file.path(input_dir_human_dog,"Bifidobacterium_longum_gene_terms.tsv"),
  file.path(input_dir_human_cat,"Bifidobacterium_longum_gene_terms.tsv")
)
...
file_paths_caerofaciens <- c(
  file.path(input_dir_cat_dog,"Collinsella_aerofaciens_gene_terms.tsv"),
  file.path(input_dir_human_dog,"Collinsella_aerofaciens_gene_terms.tsv"),
  file.path(input_dir_human_cat,"Collinsella_aerofaciens_gene_terms.tsv")
)
...
cat_dog_Mels = as.data.frame(data.table::fread(file.path(input_dir_cat_dog,"Megasphaera_elsdenii_gene_terms.tsv"), header=T, check.names = F))
...
###Bifido pullorum
cat_dog_Bpullorum = as.data.frame(data.table::fread(file.path(input_dir_cat_dog,"Bifidobacterium_pullorum_gene_terms.tsv"), header=T, check.names = F))
...
cat_dog_Lacido = as.data.frame(data.table::fread(file.path(input_dir_cat_dog,"Lactobacillus_acidophilus_gene_terms.tsv"), header=T, check.names = F))
...
human_cat_Aintestini = as.data.frame(data.table::fread(file.path(input_dir_human_cat,"Acidaminococcus_intestini_gene_terms.tsv"), header=T, check.names = F))
...
human_cat_Gform = as.data.frame(data.table::fread(file.path(input_dir_human_cat,"Gemmiger_formicilis_gene_terms.tsv"), header=T, check.names = F))
...
#Blautia obeum
human_cat_Bobeum = as.data.frame(data.table::fread(file.path(input_dir_human_cat,"Blautia_obeum_gene_terms.tsv"), header=T, check.names = F))
...
cat_dog_Rgnavus_heat = as.data.frame(data.table::fread(file.path(input_dir_cat_dog,"filtered_Ruminococcus_gnavus.tsv"), header=T, check.names = F))
...
human_dog_Rgnavus_heat = as.data.frame(data.table::fread(file.path(input_dir_human_dog,"filtered_Ruminococcus_gnavus.tsv"), header=T, check.names = F))
...
human_cat_Rgnavus_heat = as.data.frame(data.table::fread(file.path(input_dir_human_cat,"filtered_Ruminococcus_gnavus.tsv"), header=T, check.names = F))
...
gene_annot = as.data.frame(data.table::fread("/home/tobynb/shared-folder/tbranck/Pets_pooled_analysis/inputs/gene_annotation_pets.tsv", header=T, check.names = F))

#cat_dog,human_dog, and human_cat genes with largest effect size
cat_dog_rgnavus = as.data.frame(data.table::fread(file.path(input_dir_cat_dog,"Ruminococcus_gnavus_gene_terms.tsv"), header=T, check.names = F))
human_dog_rgnavus = as.data.frame(data.table::fread(file.path(input_dir_human_dog,"Ruminococcus_gnavus_gene_terms.tsv"), header=T, check.names = F))
human_cat_rgnavus = as.data.frame(data.table::fread(file.path(input_dir_human_cat,"Ruminococcus_gnavus_gene_terms.tsv"), header=T, check.names = F))
...
cat_dog_rgnavus = comb_rgnavus[1:4496,1:7]
human_dog_rgnavus = comb_rgnavus[4497:5937,1:7]
human_cat_rgnavus = comb_rgnavus[5938:10985,1:7]
...
cat_dog_rgnavus_topgenes = cat_dog_rgnavus_for_vis$gene[1:500]

human_cat_rgnavus_for_vis = human_cat_rgnavus[order(abs(human_cat_rgnavus$estimate), decreasing = TRUE),]
human_cat_rgnavus_topgenes = human_cat_rgnavus_for_vis$gene[1:500]

human_dog_rgnavus_for_vis = human_dog_rgnavus[order(abs(human_dog_rgnavus$estimate), decreasing = TRUE),]
human_dog_rgnavus_topgenes = human_dog_rgnavus_for_vis$gene[1:500]

top_genes = union(cat_dog_rgnavus_topgenes,human_dog_rgnavus_topgenes)
top_genes = union(top_genes, human_cat_rgnavus_topgenes)
write.table(top_genes,file=file.path(output_dir,"top50ea_differential_genes_anpan.tsv"),row.names=FALSE,col.names = FALSE,sep="\t",quote = FALSE)
...
cairo_pdf(file.path(output_dir,"Rgnavus_Anpan_heatmap_top50.pdf"),width = 14,height = 18)
...
cairo_pdf(file.path(output_dir,"Rgnavus_Anpan_heatmap_Glycosyl.pdf"),width = 18,height = 10)
